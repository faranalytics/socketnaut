...
export class ServiceAgent extends Agent {
  ...
  constructor(port: threads.MessagePort, options: ServiceAgentOptions) {
    super(port);
    this.agentDescription = `Thread: ${threads.threadId.toString()}`;
  ...
  protected postListeningMessage = (): void => {
    ...
    this.log.debug(`Server thread ${threads.threadId.toString()} is listening on ${JSON.stringify(this.addressInfo)}.`);
    this.register("socketConnectOpts", () => socketConnectOpts);
  };

  protected describeError = (err: unknown): string => {
    return err instanceof Error ? (err.stack ?? err.message) : "Error";
  };

  public requestProxySocketAddressInfo = async (socket: net.Socket): Promise<ProxySocketAddressInfo> => {
    ...
    const clientProxyAddressInfo = await this.call<ProxySocketAddressInfo>(
      "requestProxyAddressInfo",
      proxyServerAddressInfo
    );
...
export function createServiceAgent(options: ServiceAgentOptions): ServiceAgent {
  if (!serviceAgent) {
    if (threads.parentPort) {
      ...
      throw new Error(`The MessagePort for Worker thread ${threads.threadId.toString()} is null.`);
    }
    return serviceAgent;
  } else {
    throw new Error(`A ServiceAgent has already been instantiated for Worker thread ${threads.threadId.toString()}.`);
  }
}
