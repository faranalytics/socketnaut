...
setInterval(() => {
  httpServiceProxy.log.info(
    `proxy.agents.length: ${httpServiceProxy.agents.length.toString()}, proxy.maxWorkers: ${httpServiceProxy.maxWorkers?.toString() ?? ""}, proxy.minWorkers: ${httpServiceProxy.minWorkers.toString()}.`
  );

  httpServiceProxy.log.info(
    `Connection Distribution: ${JSON.stringify(httpServiceProxy.agents.map<number>((value: WorkerAgent) => value.connections))}`
  );

  httpsServiceProxy.log.info(
    `proxy.agents.length: ${httpsServiceProxy.agents.length.toString()}, proxy.maxWorkers: ${httpsServiceProxy.maxWorkers?.toString() ?? ""}, proxy.minWorkers: ${httpsServiceProxy.minWorkers.toString()}.`
  );

  httpsServiceProxy.log.info(
    `Connection Distribution: ${JSON.stringify(httpsServiceProxy.agents.map<number>((value: WorkerAgent) => value.connections))}`
  );
}, 1500).unref();
